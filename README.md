...
Hyprland) are missing, their targets are simply left commented out.
`install.sh` never installs distro packages, never runs the oh-my-zsh
installer and never edits shell rc files — package ownership belongs to
your distro or to `linux-setup`.
...
`install.sh` copies to `~/.config/hypr/wallpapers/`. Every other theme gets a
wallpaper generated by `wallgen` instead.

Ten of the generated ones have curated replacements that are distributed
separately rather than committed here. If you have that payload (a directory or
a zip containing `monolith-dark.png`, `monolith-light.png`, `carbon.png`,
`paper.png`, `terminal-green.png`, `ibm-ish.png`, `concrete.png`, `ink.png`,
`snow.png`, `obsidian-mono.png`), install it with:

```sh
tools/install-curated-wallpapers.sh PATH_TO_ZIP_OR_DIRECTORY
```

Wallpapers bind by filename, so `theme monolith-dark` picks up
`monolith-dark.png` automatically afterwards.
