...
QtObject {
    ...
    property var _metrics: ({})
    property var _patterns: ({})
    property int _version: 0
    function applyEngine(name, colorsJson, isDark, font, metricsJson, patternsJson) { engineName = name || "Built-in"; fontFamily = font || ""; dark = isDark; try { _engine = colorsJson ? JSON.parse(colorsJson) : ({}) } catch (e) { _engine = ({}) }; try { _metrics = metricsJson ? JSON.parse(metricsJson) : ({}) } catch (e) { _metrics = ({}) }; try { _patterns = patternsJson ? JSON.parse(patternsJson) : ({}) } catch (e) { _patterns = ({}) }; _version++ }
    ...
    function metric(name, fallback) { const version = _version; return version >= 0 && _metrics[name] !== undefined ? _metrics[name] : fallback }
    readonly property int radiusSmall: metric("radius_control", 6)
    readonly property int radiusMedium: metric("radius_surface", 10)
    readonly property int radiusLarge: metric("radius_overlay", radiusMedium + 4)
    readonly property int spacingSmall: metric("spacing_sm", 8)
    readonly property int spacingMedium: metric("spacing_md", 12)
    readonly property int spacingLarge: metric("spacing_lg", 16)
    readonly property int spacingXLarge: metric("page_padding", 24)
}
