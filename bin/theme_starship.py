...
_G = {
    ...
    "arrow_cap_left": chr(0xe0b2),   # powerline hard divider, point faces left
    "arrow_cap_right": chr(0xe0b0),  # powerline hard divider, point faces right
...
format = "[{t('git_end')}](fg:accent2)"'''
    ...
    # Right-side telemetry floats directly on the terminal background: no
    # Powerline cap and no filled capsule, just semantic-coloured indicators.
    right = "".join([
...
format = "[ {t('duration_icon')} $duration ](bold fg:fg)"

[status]
disabled = {str(not on('cmd_status_enabled')).lower()}
success_symbol = ""
format = "[ {t('cmd_status_icon')} $status ](bold fg:urgent)"
